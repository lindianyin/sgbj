...
Database::Database(const std::string& h,const std::string& u,const std::string& p,const std::string& d,unsigned long option,unsigned int port, IError *e)
:host(h)
,user(u)
,password(p)
,database(d)
...
Database::Database(Mutex& m,const std::string& h,const std::string& u,const std::string& p,const std::string& d,unsigned long option,unsigned int port, IError *e)
:host(h)
,user(u)
,password(p)
,database(d)
...
Database::OPENDB *Database::grabdb()
{
	...
	if (!odb)
	{
		...
		if (m_embedded)
		{
			if (!mysql_real_connect(&odb -> mysql,NULL,NULL,NULL,database.c_str(),m_port,NULL,m_option) )
			{
				printf("mysql_real_connect(NULL,NULL,NULL,%s,0,NULL,0) failed - list size %ld, error:%s",database.c_str(),m_opendbs.size(),mysql_error(&odb->mysql));
				delete odb;
				return NULL;
			}
		}
		...
		{
			if (!mysql_real_connect(&odb -> mysql,host.c_str(),user.c_str(),password.c_str(),database.c_str(),m_port,NULL,m_option) )
			{
				printf("mysql_real_connect(NULL,NULL,NULL,%s,0,NULL,0) failed - list size %ld, error:%s",database.c_str(),m_opendbs.size(),mysql_error(&odb->mysql));
				delete odb;
				return NULL;
...
void Database::KeepAlive()
{
	...
	for (opendb_v::iterator it = m_opendbs.begin(); it != m_opendbs.end(); )
	{
		odb = *it;
		if (!odb -> busy)
		{
