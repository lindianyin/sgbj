...
{		// query, no result
		...
		uint64_t start = 1000*1000*_tstart.tv_sec + _tstart.tv_usec;
#endif
...
#ifdef DEBUB_QUERY_TIME
			gettimeofday(&_tstart, NULL);
			uint64_t cost = 1000*1000*_tstart.tv_sec + _tstart.tv_usec - start;
			if (cost > DEBUB_QUERY_TIME)
			{
				std::cout<<"execute <"<<sql<<"> cost "<<(double)cost/1000<<"'ms"<<std::endl;
			}
#endif
