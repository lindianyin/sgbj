...
static int bankMinLeft(char_bank_list& b)
{
    int min_left = 0;
    for (size_t i = 0; i < b.size(); ++i)
    {
        ...
        if (bc.get())
        {
            for (size_t j = 0; j < iBankSubCaseCount; ++j)
            {
                if (bc->m_feedback_state[j] == 0)
                {
                    if (bc->m_state == 2 && bc->m_sub_can_get_time[j] > 0)
                    {
                        int left = bc->m_sub_can_get_time[j] - time(NULL);
                        if (min_left == 0 || left < min_left)
                        {
                            min_left = left;
                        }
                    }
...
boost::shared_ptr<char_bank_list> bankMgr::open(int cid)
{
    ...
    {
        cdata->updateTopButton(top_button_bank, 1);
    }
    return p_cb;
}
...
int ProcessGetBankSilver(net::session_ptr& psession, json_spirit::mObject& o, json_spirit::Object& robj)
{
    ...
    robj.push_back( Pair("bank_silver", cdata->BankSilver()) );
    return HC_SUCCESS;
}
...
int ProcessDealBankSilver(net::session_ptr& psession, json_spirit::mObject& o, json_spirit::Object& robj)
{
    ...
    int purpose = 1, silver = 0;
    READ_INT_FROM_MOBJ(purpose, o, "purpose");
    READ_INT_FROM_MOBJ(silver, o, "silver");
    robj.push_back( Pair("purpose", purpose) );
    if (purpose == 1)
    {
        robj.push_back( Pair("silver", silver) );
        if (cdata->addBankSilver(silver) >= 0)
            return HC_SUCCESS;
    }
    else if(purpose == 2)
    {
        int cost = silver * 2 / 100;
        robj.push_back( Pair("silver", silver-cost) );
        if (cdata->subBankSilver(silver) >= 0)
            return HC_SUCCESS;
    }
    return HC_ERROR;
}
...
int ProcessGetBankCaseState(net::session_ptr& psession, json_spirit::mObject& o, json_spirit::Object& robj)
{
    ...
    if (p_cb.get())
    {
        ...
        if (state == 0)
        {
            int min_left = bankMinLeft(*p_cb);
            robj.push_back( Pair("min_left", min_left) );
        }
    ...
    if (p_cb.get())
    {
        for (int i = 0; i < (int)(*p_cb).size(); ++i)
        {
            ...
            if ((*p_cb)[i].get() && (*p_cb)[i]->m_case_id == case_id && (*p_cb)[i]->m_state == 1)
            {
                ...
                {
                    if (cdata->subGold((*p_cb)[i]->m_base->_need_gold, gold_cost_for_bank, true) < 0)
                    {
