...
int loadAccountFromDb(const std::string& account)
{
    ...
    Query q(GetDb());
#ifdef ONE_CHARACTOR
    q.get_result("SELECT c.id,c.lastlogin,c.spic,c.level,c.name FROM `charactors` as c left join char_data as cd on c.id=cd.cid WHERE c.account='" + GetDb().safestr(account) + "' order by id limit 1");
#else
    q.get_result("SELECT c.id,c.lastlogin,c.spic,c.level,c.name FROM `charactors` as c left join char_data as cd on c.id=cd.cid WHERE c.account='" + GetDb().safestr(account) + "' order by lastlogin desc");
#endif
    CHECK_DB_ERR(q);
    if (q.num_rows())
    {
