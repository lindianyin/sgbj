...
#ifdef VN_EN_SERVER
        //活动更新
        if (Singleton<new_event_mgr>::Instance().isEventOpen(event_turn_horse))
        {
            Singleton<new_event_mgr>::Instance().updateEvent(cData.m_id,event_turn_horse,cData.m_horse.horse->id);
        }
#endif
    }
    return HC_SUCCESS;
...
#ifdef VN_EN_SERVER
    //活动更新
    if (Singleton<new_event_mgr>::Instance().isEventOpen(event_train_horse))
    {
        int tmp = cData.queryExtraData(char_data_type_normal, char_data_event_train_horse_cnt);
        cData.setExtraData(char_data_type_normal, char_data_event_train_horse_cnt, ++tmp);
        Singleton<new_event_mgr>::Instance().updateEvent(cData.m_id,event_train_horse,tmp);
    }
#endif
    return HC_SUCCESS;
}
