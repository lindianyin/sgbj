...
//测试用户防沉迷测试
extern int getTestAccountChenmiState(const std::string& account);
            ...
            if (psession->state() == STATE_CONNECTED)
            {
                ...
                if (account != "")
                {
                    ...
                    else
                    {
                        ...
                        if (GeneralDataMgr::getInstance()->isChenmiEnable())
                        {
                            task._isAdult = getTestAccountChenmiState(task.username);
                        }
                        else
                        {
                            task._isAdult = 1;
                ...
                //cout<<"login msg "<<task._qid<<","<<task._is_qq_yellow<<"|"<<task._qq_yellow_level<<"|"<<task._is_qq_year_yellow<<endl;
#ifdef QQ_PLAT
