...
{
    event_race_win = 1,
    event_attack_power = 2,
    event_train_horse = 3,
    event_turn_horse = 4,
    event_general_star = 5,
    event_max = 5,
};
...
struct template_event
{
    ...
    int checkMore(int value);
};

struct char_event_data
{
    template_event& m_handle;
    int m_cid;
    int m_canGet;
    std::map<int, int> m_geted;
    std::vector<int> m_finish_list;

    char_event_data(template_event& h, int cid);
    int getAwards(CharData& cdata, int need, json_spirit::Object& robj);
...
struct login_event
{
    time_t m_start_time;
    time_t m_end_time;
    int m_cur_state;//登录活动有三个阶段，分别是第一天，第四天，第七天开始
    int m_cur_day;//当前是第几天
    std::map<int, boost::shared_ptr<baseLibao> > m_rewards;
    bool isOpen();
    bool inList(int day);
};
...
{
    login_event& m_handle;
    int m_cid;
    int m_canGet;
    std::map<int, int> m_geted;
    std::vector<int> m_finish_list;

    char_login_data(login_event& h, int cid);
    int getAwards(CharData& cdata, int day, json_spirit::Object& robj);
    void check(CharData& cdata);
    void save();
};
#endif
...
private:
    ...
    /**************活动****************/
public:
    template_event* getEventTemp(int type);
    bool isEventOpen(int type);
    int getEvent(CharData* pc, int type, json_spirit::Object& robj);
    int getReward(CharData* pc, int type, int need, json_spirit::Object& robj);
    char_event_data* getCharEventData(int cid, int type);
    void updateEvent(int cid, int type, int value);
private:
    std::map<int, boost::shared_ptr<template_event> > m_events;
    std::map<std::pair<int,int>, boost::shared_ptr<char_event_data> > m_all_char_event_data;

    /**************登录活动****************/
public:
    bool isLoginEventOpen();
    void getLoginEvent(CharData* pc, char_login_data& e, json_spirit::Object& robj);
    char_login_data* getCharLoginEventData(int cid);
    void checkLoginEvent(CharData* pc);
    void updateLoginEvent();
private:
    login_event m_login_event;
    std::map<int, boost::shared_ptr<char_login_data> > m_char_login_data;
#endif
};
