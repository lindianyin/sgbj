...
#ifdef VN_EN_SERVER
bool CharData::canGetFacebookPresent()
{
    return queryExtraData(char_data_type_normal,char_data_fb_get) == 0;
}

int CharData::getFacebookPresentInfo(json_spirit::Object& robj)
{
    //领取奖励
    std::list<Item> items;
    Item tmp1(item_type_treasure, treasure_type_silver_card, 10, 0);
    items.push_back(tmp1);
    Item tmp2(item_type_treasure, treasure_type_gold_card, 10, 0);
    items.push_back(tmp2);
    Item tmp3(item_type_treasure, treasure_type_honor_card, 20, 0);
    items.push_back(tmp3);
    json_spirit::Array getlist;
    itemlistToArray(items,getlist);
    robj.push_back( Pair("can_get", canGetFacebookPresent()));
    robj.push_back( Pair("getlist", getlist));
    return HC_SUCCESS;
}

int CharData::getFacebookPresent()
{
    if (canGetFacebookPresent())
    {
        //领取奖励
        std::list<Item> items;
        Item tmp1(item_type_treasure, treasure_type_silver_card, 10, 0);
        ...
        Item tmp3(item_type_treasure, treasure_type_honor_card, 20, 0);
        items.push_back(tmp3);
        std::string present_info = itemlistToAttach(items);
        sendSystemMail(m_name, m_id, strFacebookMailTitle, strFacebookMailContent, present_info);
        setExtraData(char_data_type_normal,char_data_fb_get,1);
        boost::shared_ptr<OnlineCharactor> account = GeneralDataMgr::getInstance()->GetOnlineCharactor(m_name);
        if (account.get())
        {
            account->Send("{\"type\":"+LEX_CAST_STR(top_level_event_facebook)+",\"cmd\":\"removeAction\",\"s\":200}");
        }
        return HC_SUCCESS;
    }
    return HC_ERROR;
...
#ifdef VN_EN_SERVER
                    //活动更新
                    if (Singleton<new_event_mgr>::Instance().isEventOpen(event_general_star))
                    {
                        Singleton<new_event_mgr>::Instance().updateEvent(m_charData.m_id,event_general_star,it->second->m_chengzhang_star->id);
                    }
#endif
                }
            }
...
#ifdef VN_EN_SERVER
        //活动更新
        if (Singleton<new_event_mgr>::Instance().isEventOpen(event_attack_power))
        {
            Singleton<new_event_mgr>::Instance().updateEvent(m_charData.m_id,event_attack_power,total_power);
        }
#endif
    }
    return total_power;
...
int GeneralDataMgr::CharactorLogin(boost::shared_ptr<OnlineCharactor> oc)
{
    if (oc.get() && oc->m_account.get() && oc->m_charactor.get())
    {
        ...
        //距离上次登录时间过去几天了
        {
            ...
            if (days_passed >= 5)
            {
                //5天回流奖励800金币1000万银币2000军令5000声望
                add_gold = 800;
                add_silver = 10000000;
                add_ling = 2000;
                add_prestige = 5000;
            ...
            if (days_passed >= 21)
            {
                //21天回流奖励
                add_gold = 200;
                add_silver = oc->m_charactor->m_level*30000;
                add_prestige = 3000;
            }
            else if (days_passed >= 3)
            {
                //3天回流奖励
                add_silver = oc->m_charactor->m_level*3000;
                add_prestige = 300;
            }
#endif
            if (add_prestige)
            {
                oc->m_charactor->addPrestige(add_prestige);
                reward_content += LEX_CAST_STR(add_prestige) + strPrestige;
            }
            if (add_silver)
            {
                oc->m_charactor->addSilver(add_silver);
                reward_content += "," + LEX_CAST_STR(add_silver) + strSilver;
            }
            if (add_gold)
            {
                oc->m_charactor->addGold(add_gold);
                //金币获得统计
                add_statistics_of_gold_get(oc->m_charactor->m_id,oc->m_charactor->m_ip_address,100,gold_get_gift, oc->m_charactor->m_union_id, oc->m_charactor->m_server_id);
                reward_content += "," + LEX_CAST_STR(add_gold) + strGold;
            }
            if (add_ling)
            {
                oc->m_charactor->addLing(add_ling);
                reward_content += "," + LEX_CAST_STR(add_ling) + strLing;
            }
            if (add_yushi)
            {
                ...
                {
                    int err_code = 0;
                    oc->m_charactor->m_bag.addGem(treasure_type_yushi, add_yushi, err_code);
                    reward_content += "," + LEX_CAST_STR(add_yushi) + tr->name;
                }
            }
            if (reward_content != "")
            {
                std::string strMail = strCharBackMailContent;
                str_replace(strMail, "$D", LEX_CAST_STR(days_passed));
                str_replace(strMail, "$T", reward_content);
                sendSystemMail(oc->m_charactor->m_name, oc->m_charactor->m_id, strCharBackMailTitle, strMail);
            }
...
#ifdef VN_EN_SERVER
        //活动更新
        if (Singleton<new_event_mgr>::Instance().isLoginEventOpen())
        {
            Singleton<new_event_mgr>::Instance().checkLoginEvent(oc->m_charactor.get());
        }
#endif
        //更新登录时间
...
int CreateChar(const std::string& account, int union_id, const std::string& server_id,  const std::string& qid, int camp, int spic, const std::string& xname, int g1, int g2, uint64_t& cid, int inv_id)
{
    ...
    job.sqls.push_back("insert into char_data set cid='" + LEX_CAST_STR(cid)
            ...
            + "',backPack=0,freeRest='5',vip='3',chat='" + GetDb().safestr(strCharChatMessage[spic-1]) + "',cLevel=1,exp='1000',official=1");
    #else
    job.sqls.push_back("insert into char_data set cid='" + LEX_CAST_STR(cid)
            + "',camp='" + LEX_CAST_STR(camp)
